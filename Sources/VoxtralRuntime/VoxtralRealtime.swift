...
public extension VoxtralRealtimeModel {
    static func fromDirectory(
        ...
        let initStart = Date()
        progressHandler?("Reading model config")
        let configURL = modelDir.appendingPathComponent("config.json")
        let configData = try Data(contentsOf: configURL)
        let config = try JSONDecoder().decode(VoxtralRealtimeConfig.self, from: configData)

        progressHandler?("Initializing model: constructing module graph")
        let model = VoxtralRealtimeModel(config)
        progressHandler?("Initializing model: module graph ready (\(String(format: "%.1fs", Date().timeIntervalSince(initStart))))")
        progressHandler?("Initializing model: loading tokenizer")
        ...
        progressHandler?("Initializing model: tokenizer ready (\(String(format: "%.1fs", Date().timeIntervalSince(initStart))))")

        progressHandler?("Discovering weight files")
        let files = try FileManager.default.contentsOfDirectory(at: modelDir, includingPropertiesForKeys: nil)
        let safetensors = files.filter { $0.pathExtension == "safetensors" }.sorted { $0.lastPathComponent < $1.lastPathComponent }

        var weights: [String: MLXArray] = [:]
        for (index, file) in safetensors.enumerated() {
            progressHandler?("Loading weights \(index + 1)/\(safetensors.count)")
        ...
        progressHandler?("Model runtime ready")
